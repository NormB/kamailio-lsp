...
```json title="Set assistance parameter"
{ "assistance": false }
```
...
at any point: hover and completion text is parsed from Kamailio's own
documentation on disk rather than generated.

Everything it touches is local:

- **Read** — the open configuration and every file its
  `include_file`/`import_file` closure names, plus the configured `kamailioSrc` tree and `kamailioWiki` checkout.
...
- **Executed** — the `kamailioPath` binary, once per check, under the
