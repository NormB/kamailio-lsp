...
git clone --depth 1 https://github.com/NormB/kamailio-lsp.git "$ROOT/checkout"
cp -r "$ROOT/checkout/tree-sitter-kamailio" "$ROOT/grammar"
git -C "$ROOT/grammar" init -q
git -C "$ROOT/grammar" add -A
git -C "$ROOT/grammar" -c user.email=you@example.com -c user.name=you \
    commit -qm "tree-sitter-kamailio"
