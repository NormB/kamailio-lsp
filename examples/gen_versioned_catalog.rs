...
//!         5.8.8=/path/to/5.8.8 6.0.7=/path/to/6.0.7 6.1.4=/path/to/6.1.4 \
...
//! them as `KAMAILIO_LSP_TEST_TREES` in this same `<version>=<path>`
...
fn main() {
    ...
    let mut harvests: Vec<(String, Vec<kamailio_lsp::catalog::ModuleDoc>)> = Vec::new();
    for arg in &args {
        let Some((version, tree)) = arg.split_once('=') else {
            eprintln!("expected <version>=<tree>, got '{arg}'");
            std::process::exit(2);
        };
        let mut modules = kamailio_lsp::catalog::harvest_tree(std::path::Path::new(tree));
        if modules.is_empty() {
            eprintln!("'{tree}' yields no module documentation");
            std::process::exit(2);
        }
        // the delta is only meaningful over a canonical order
        kamailio_lsp::catalog::canonicalize(&mut modules);
        eprintln!(
    ...
    let deltas = harvests
        .windows(2)
        .map(|pair| kamailio_lsp::catalog::diff_catalogues(&pair[0].1, &pair[1].1, &pair[1].0))
        .collect();
    let out = kamailio_lsp::catalog::VersionedModules {
        base: kamailio_lsp::catalog::BuiltinModules {
