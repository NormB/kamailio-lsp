# kamailio-lsp one-command installer for Windows (PowerShell).
#
...
# to %LOCALAPPDATA%\kamailio-lsp, and installs the VS Code extension
...
Write-Host "Installing kamailio-lsp $tag for $arch ..."

$dest = Join-Path $env:LOCALAPPDATA 'kamailio-lsp'
New-Item -ItemType Directory -Force -Path $dest | Out-Null
$zip = Join-Path $env:TEMP 'kamailio-lsp.zip'
Invoke-WebRequest "https://github.com/$repo/releases/download/$tag/kamailio-lsp-$tag-$arch-windows.zip" -OutFile $zip
Expand-Archive -Path $zip -DestinationPath $dest -Force
Remove-Item $zip
Write-Host "Server installed: $dest\kamailio-lsp.exe"
Write-Host "NOTE: add $dest to your PATH if editors other than VS Code need it."

$vsix = Join-Path $env:TEMP 'kamailio-lsp-ext.vsix'
$installed = $false
foreach ($editor in 'code', 'code-insiders', 'codium') {
    if (Get-Command $editor -ErrorAction SilentlyContinue) {
        if (-not (Test-Path $vsix)) {
            Invoke-WebRequest "https://github.com/$repo/releases/download/$tag/kamailio-lsp-ext-$tag.vsix" -OutFile $vsix
        }
...
if (-not $installed) {
    ...
    Write-Host "  1. Download: https://github.com/$repo/releases/download/$tag/kamailio-lsp-ext-$tag.vsix"
