...
//! that has set no wiki checkout — the state every user is in on the day
...
fn boot(
    ...
    let dir = std::env::temp_dir().join(format!("kalsp-builtin-{tag}-{}", std::process::id()));
    ...
    // no wiki checkout, no binary: the out-of-the-box state
...
/// nothing but control-flow keywords until a wiki checkout was set.
#[test]
fn a_global_parameter_completes_with_no_wiki_checkout_configured() {
    ...
    assert!(
        labels.contains(&"log_facility".to_string()),
        "log_facility must complete out of the box; got {} items: {:?}",
        labels.len(),
...
fn hover_on_a_built_in_entry_names_the_version_and_the_escape_hatch() {
    let (mut child, rx, mut stdin, uri, _) = boot("hover", "log_facility=LOG_LOCAL0\n");
    write_msg(
        &mut stdin,
    ...
    let text = r["result"]["contents"]["value"]
        ...
        .to_string();
    assert!(
        text.contains("Built-in documentation from Kamailio"),
        "hover must disclose the provenance: {text:?}"
    );
    assert!(
        text.contains("kamailioWiki"),
        "hover must name the setting that overrides it: {text:?}"
    );
    let _ = child.kill();
}

/// `is_method` is a `textops` function, and it was the case that
...
fn a_module_function_completes_with_no_source_tree_configured() {
    let (mut child, rx, mut stdin, uri, ready) =
        boot("modfn", "loadmodule \"textops.so\"\nroute {\n    is_\n}\n");
    assert!(
...
fn a_module_function_stays_hidden_until_its_module_is_loaded() {
    ...
    assert!(
        !labels.contains(&"is_method".to_string()),
        "textops is not loaded, so is_method must not be offered"
    );
...
/// With nothing configured there were no module NAMES either, so the first thing
...
fn module_names_complete_inside_loadmodule_with_nothing_configured() {
    ...
    assert!(
        labels.iter().any(|l| l.starts_with("textops")),
        "textops must be offerable: {:?}",
...
/// written down here.  In 6.1 it happens to be `log_facility`, which is
/// both a core global and an `acc` modparam — but whether a
...
fn colliding_param() -> (String, String) {
    ...
    let core: HashSet<&str> = kamailio_lsp::catalog::builtin_core()
        .core
        .params
        .iter()
        .map(|p| p.name.as_str())
        .collect();
    for m in &kamailio_lsp::catalog::builtin_modules().modules {
