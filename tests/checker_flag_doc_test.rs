...
//!
//! `kamailio` accepts `-c` and has no uppercase form at all; OpenSIPS
//! accepts both and they run different checks.  These two servers are
//! sibling repositories whose prose gets copied between them, and the
//! uppercase flag reached `SECURITY.md` and the Zed setup guide —
//! telling a reader to run something this binary rejects outright —
//! while every behavioural test stayed green.  That is the whole gap:
//! the argv is gated (`cli_test`, `e2e_test` both assert it), the
//! prose describing the argv was not.
//!
