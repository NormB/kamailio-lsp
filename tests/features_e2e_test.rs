...
//! stdio LSP against a SYNTHETIC Kamailio tree + wiki — hermetic, no
//! gating.
...
fn mk_tree(root: &std::path::Path) {
    ...
    w(
        root.join("src/modules/mymod/README"),
        "MyMod Module\n\n2. Parameters\n\n2.1. my_param (string)\n\n   A parameter.\n\n3. Functions\n\n3.1.  my_func(arg)\n\n   Does things.\n",
    );
    w(
        root.join("src/modules/othermod/README"),
        "OtherMod Module\n\n2. Functions\n\n2.1.  other_func()\n\n   Other things.\n",
    );
}

fn mk_wiki(root: &std::path::Path) {
...
fn all_features_over_stdio_against_a_synthetic_tree() {
    let base = std::env::temp_dir().join(format!("kamlsp-feat-{}", std::process::id()));
    ...
    write_msg(
        ...
        &serde_json::json!({"jsonrpc":"2.0","id":7,"method":"textDocument/completion","params":{
            "textDocument":{"uri":uri},"position":{"line":8,"character":19}}}),
    );
...
fn signature_help_and_pvar_text_edits() {
    let base = std::env::temp_dir().join(format!("kamlsp-sig-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&base);
    let tree = base.join("tree");
    mk_tree(&tree);
    let wiki = base.join("wiki");
...
fn undocumented_modparam_warns_between_saves() {
    let base = std::env::temp_dir().join(format!("kamlsp-catv-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&base);
    let tree = base.join("tree");
    ...
    let broken = "loadmodule \"mymod.so\"\nmodparam(\"mymod\", \"bad_param\", 1)\nrequest_route {\n    exit;\n}\n";
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","method":"textDocument/didChange","params":{
        "textDocument":{"uri":uri,"version":2},
