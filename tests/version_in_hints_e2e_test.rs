...
fn hover_text_at(tag: &str, opts: serde_json::Value, body: &str, line: u32, ch: u32) -> String {
    let dir = std::env::temp_dir().join(format!("kamlsp-vhints-{tag}-{}", std::process::id()));
...
fn with_the_setting_on_a_hover_names_the_release_and_the_escape_hatch() {
    ...
    assert!(
        text.contains("Built-in core documentation from Kamailio 6.1.x"),
...
fn a_core_entry_names_its_own_release_not_the_selected_one() {
    let text = hover_text(
        "selected",
        serde_json::json!({"versionInHints": true, "kamailioVersion": "6.0.7"}),
    );
    assert!(
        text.contains("Built-in core documentation from Kamailio 6.1.x"),
    ...
    assert!(
        !text.contains("core documentation from Kamailio 6.0.7"),
...
/// carries several releases and follows `kamailioVersion`; core
...
fn a_module_entry_follows_the_selected_release_while_core_does_not() {
    // `log_level` would be the obvious core parameter to hover, but
    // Kamailio's `acc` module exports one of that name too and the
    // module entry wins — which is precisely the confusion this test
    // exists to pin down.
    let body = "advertised_address=\"1.2.3.4\"\nloadmodule \"usrloc.so\"\nmodparam(\"usrloc\", \"db_mode\", 0)\n";
    let opts = serde_json::json!({"versionInHints": true, "kamailioVersion": "6.0.7"});

    // the module parameter: follows the selection
    let module = hover_text_at("kinds-mod", opts.clone(), body, 2, 22);
    assert!(
        module.contains("module documentation from Kamailio 6.0.7"),
...
fn selecting_a_release_leaves_the_core_catalogue_alone() {
    ...
    let older = hover_text_at(
        "core-older",
        serde_json::json!({"versionInHints": true, "kamailioVersion": "5.8.8"}),
        "advertised_address=\"1.2.3.4\"\n",
        0,
        3,
    );
    assert!(
        newest.contains("core documentation from Kamailio"),
