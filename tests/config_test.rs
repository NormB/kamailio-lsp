use kamailio_lsp::logic::{diag_matches_file, resolve_bin};

#[test]
fn absent_everywhere_defaults_to_path_lookup() {
    assert_eq!(resolve_bin(None, None), Some("kamailio".to_string()));
}
...
fn option_beats_env() {
    assert_eq!(
        resolve_bin(Some("/a/kamailio"), Some("/b/kamailio".into())),
        Some("/a/kamailio".to_string())
    );
    assert_eq!(
        resolve_bin(None, Some("/b/kamailio".into())),
        Some("/b/kamailio".to_string())
    );
}
...
fn timeout_resolution_option_env_default() {
    use kamailio_lsp::logic::resolve_timeout;
...
fn existing_same_basename_files_do_not_cross_attach() {
    ...
    let base = std::env::temp_dir().join(format!("kamlsp-canon-{}", std::process::id()));
...
fn symlinked_spellings_of_the_same_file_match_canonically() {
    let base = std::env::temp_dir().join(format!("kamlsp-link-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&base);
