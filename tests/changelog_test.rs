...
///
/// `kamailioVersion` is a dropdown in the extension's settings, and its
...
fn the_version_dropdown_offers_exactly_the_shipped_releases() {
    ...
    let offered: Vec<String> =
        pkg["contributes"]["configuration"]["properties"]["kamailioLsp.kamailioVersion"]["enum"]
            .as_array()
    ...
    let shipped: Vec<String> = kamailio_lsp::catalog::builtin_versioned()
        .versions()
        .into_iter()
        .map(str::to_string)
        .collect();
    ...
    let described = pkg["contributes"]["configuration"]["properties"]
        ["kamailioLsp.kamailioVersion"]["enumDescriptions"]
        .as_array()
