...
fn boot(
    ...
    let base = std::env::temp_dir().join(format!("kamlsp-watch-{tag}-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&base);
    std::fs::create_dir_all(base.join("tree/src/modules/mymod")).unwrap();
    std::fs::write(
        base.join("tree/src/modules/mymod/README"),
        "MyMod Module\n\n2. Functions\n\n2.1.  first_fn()\n\n   One.\n",
    )
    .unwrap();
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_kamailio-lsp"))
        ...
        .env("KAMAILIO_LSP_SRC", base.join("tree").display().to_string())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
...
fn the_server_registers_for_the_files_it_reads() {
    ...
    assert!(
        patterns.iter().any(|p| p.contains("README")),
...
fn an_include_changing_on_disk_republishes_diagnostics() {
    ...
    write_msg(
        ...
        &serde_json::json!({"jsonrpc":"2.0","method":"textDocument/didOpen","params":{
            "textDocument":{"uri":uri,"languageId":"kamailio-cfg","version":1,"text":main}}}),
    );
...
fn a_changed_documentation_tree_is_reharvested() {
    ...
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","method":"workspace/didChangeWatchedFiles","params":{
            "changes":[{"uri": format!("file://{}", base.join("tree/src/modules/mymod/README").display()), "type": 2}]}}),
    );
