...
fn grammar_dir() -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tree-sitter-kamailio")
}
...
fn the_committed_parser_is_present_and_looks_generated() {
    ...
    assert!(
        parser.contains("tree_sitter_kamailio"),
...
fn the_committed_parser_matches_grammar_js() {
    ...
    let scratch = std::env::temp_dir().join(format!("ts-kamailio-{}", std::process::id()));
    ...
    assert_eq!(
        ...
        "src/parser.c is stale: grammar.js has changed since it was \
         generated. Run `npx tree-sitter-cli@0.26 generate` in tree-sitter-kamailio and commit the result"
    );
    assert!(
        fresh == committed,
