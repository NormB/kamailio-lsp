...
fn boot(
    ...
    let dir = std::env::temp_dir().join(format!("kamlsp-rf-{tag}-{}", std::process::id()));
...
fn duplicate_loadmodules_can_be_removed_and_the_result_is_clean() {
    ...
    assert_eq!(
        out, "loadmodule \"sl.so\"\nloadmodule \"tm.so\"\nrequest_route {\n    exit;\n}\n",
        "applied result:\n{out}"
    );
    // the surviving order is the original order: nothing was sorted
    assert!(
        out.find("sl.so").unwrap() < out.find("tm.so").unwrap(),
