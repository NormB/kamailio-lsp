...
//! to match only `kamailio.cfg` after the association widened, told
//! Helix to claim EVERY `.cfg` file — the one thing the extension
//! deliberately refuses to do — and presented `kamailioSrc` as
...
fn no_example_claims_every_cfg_file() {
    ...
    for (n, line) in doc.lines().enumerate() {
        ...
        // `kamailio*.cfg` is NOT that: the quote has to sit directly
...
fn every_environment_variable_shown_is_one_the_server_reads() {
    ...
    for entry in std::fs::read_dir(format!("{}/src", root())).unwrap() {
        ...
        for (i, _) in text.match_indices("KAMAILIO_LSP_") {
    ...
    for (i, _) in doc.match_indices("KAMAILIO_LSP_") {
...
fn every_vs_code_setting_shown_is_contributed() {
    ...
    let mut shown = BTreeSet::new();
    for (i, _) in doc.match_indices("kamailioLsp.") {
        let name: String = doc[i..]
            .chars()
            .take_while(|c| c.is_alphanumeric() || *c == '.')
            .collect();
        // prose names the prefix itself ("settings live under
        // `kamailioLsp.`"); only a prefix WITH a key is a setting
        if name.ends_with('.') {
            continue;
        }
        shown.insert(name);
...
fn the_documented_cli_usage_is_the_real_usage() {
    ...
    let start = cli.find("usage: kamailio-lsp check").expect("usage string");
