...
fn boot(
    ...
    let dir = std::env::temp_dir().join(format!("kamlsp-fmt-{tag}-{}", std::process::id()));
    ...
    let mut child = Server::new(
        Command::new(env!("CARGO_BIN_EXE_kamailio-lsp"))
            .env("KAMAILIO_LSP_BIN", "") // diagnostics off: this is about edits
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
...
fn an_already_formatted_document_produces_no_edits() {
    let (mut child, rx, mut stdin, uri) = boot("clean", "request_route {\n\texit;\n}\n");
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","id":2,"method":"textDocument/formatting","params":{
