...
//! two in `kamailio-lsp` and one in its sibling, all in strings shown to the
...
fn the_user_facing_warnings_read_as_sentences() {
    ...
    for (file, line, lit) in &lits {
        if !lit.starts_with("kamailio-lsp: ") {
            continue;
        }
        checked += 1;
        assert!(
    ...
    assert!(
        checked >= 3,
        "only {checked} 'kamailio-lsp: ' messages found — the prefix has changed"
    );
