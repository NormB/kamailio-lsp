...
use kamailio_lsp::format::{Options, format, format_lines};
...
fn range_formatting_touches_only_the_requested_lines() {
    use kamailio_lsp::format::format_range;
    let src = "route {\nexit;\n}\nroute[A] {\nexit;\n}\n";
...
#[test]
fn preprocessor_directives_keep_their_own_column() {
    // `#!` and `!!` are read line-wise by the preprocessor, ahead of
    // the parser: the formatter must not move them
    let src = "#!KAMAILIO\nrequest_route {\n#!ifdef WITH_TLS\nexit;\n#!endif\n}\n";
    let out = format(src, &spaces(2));
    let lines: Vec<&str> = out.lines().collect();
    assert_eq!(lines[0], "#!KAMAILIO");
    assert_eq!(lines[2], "#!ifdef WITH_TLS", "directive was indented");
    assert_eq!(lines[3], "  exit;", "code inside the block still indents");
    assert_eq!(lines[4], "#!endif");
}

#[test]
fn backslash_continued_directives_are_left_whole() {
    // a continued `#!define` carries its own layout across lines
    let src = "#!define LONG one \\\n    two \\\n    three\nrequest_route {\nexit;\n}\n";
    let out = format(src, &spaces(2));
    assert!(
        out.contains("#!define LONG one \\\n    two \\\n    three"),
        "a directive continuation was reindented: {out:?}"
    );
    assert!(out.contains("\n  exit;"), "code after it still indents");
}

#[test]
fn double_slash_comments_do_not_move_the_depth() {
    let src = "request_route {\n// a } comment\nexit;\n}\n";
    let out = format(src, &spaces(2));
    let lines: Vec<&str> = out.lines().collect();
    assert_eq!(lines[1], "  // a } comment");
    assert_eq!(lines[2], "  exit;", "the // brace changed the depth");
}
