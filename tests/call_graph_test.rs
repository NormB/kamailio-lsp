...
//! `route[NAME]` block — that is the whole Kamailio route-namespace
...
use kamailio_lsp::logic::{call_edges, enclosing_block};
...
fn a_call_is_attributed_to_the_block_that_contains_it() {
    ...
    assert_eq!(
        named,
        vec![
            ("request_route[]".to_string(), "RELAY".to_string()),
            ("request_route[]".to_string(), "QUOTED".to_string()),
...
fn enclosing_block_finds_the_containing_route() {
    ...
    assert_eq!(b.kind, "route", "route[RELAY] is a main-table block");
