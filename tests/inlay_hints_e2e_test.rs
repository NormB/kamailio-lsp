...
fn mk_tree(root: &std::path::Path) {
    ...
    w(
        root.join("src/modules/tm/README"),
        "TM Module\n\n2. Functions\n\n\
         2.1.  t_relay([flags],[outbound_proxy])\n\n   Relay it.\n\n\
         2.2.  t_newtran()\n\n   New transaction.\n",
    );
}
...
fn boot(
    ...
    let base = std::env::temp_dir().join(format!("kamlsp-hint-{tag}-{}", std::process::id()));
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_kamailio-lsp"))
        ...
        .env("KAMAILIO_LSP_SRC", tree.display().to_string())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
...
fn the_setting_applies_live() {
    ...
    write_msg(
        ...
        &serde_json::json!({"jsonrpc":"2.0","method":"workspace/didChangeConfiguration","params":{
            "settings":{"kamailioLsp":{"inlayHintParameterNames": true}}}}),
    );
    assert!(
...
/// Preprocessor symbols carry their expansion as a hint, so a config
/// full of `#!define`d constants reads without jumping around.  The
/// definition site is not hinted: `#!define PORT 5060` already says
/// what it binds.
#[test]
fn preprocessor_symbols_show_what_they_expand_to() {
    let base = std::env::temp_dir().join(format!("kamlsp-hint-def-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&base);
    ...
    // the `#!ifdef PORT` operand is a use even though it is not code
    assert!(
        got.iter().any(|(l, _, lab)| *l == 3 && lab == "= 5060"),
        "the #!ifdef operand should be hinted: {got:?}"
    );
    // the definition itself is not hinted
    assert!(
