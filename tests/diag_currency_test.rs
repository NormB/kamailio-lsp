...
/// Stub kamailio: behavior keyed by a mode file next to the cfg; the
/// cfg path is the LAST argument of the -c invocation.
...
echo " 0(1) CRITICAL: <core> [core/cfg.y:1]: yyerror_at(): parse error in config file $cfg, line 1, column 2-3: planted" >&2
exit 255
"#;

fn setup(tag: &str) -> (std::path::PathBuf, std::path::PathBuf, String) {
    let dir = std::env::temp_dir().join(format!("kamlsp-cur-{tag}-{}", std::process::id()));
...
fn failed_checks_clear_stale_diagnostics_and_publishes_carry_versions() {
    ...
    let mut child = Server::new(
        Command::new(env!("CARGO_BIN_EXE_kamailio-lsp"))
            ...
            .env("KAMAILIO_LSP_CHECK_TIMEOUT_MS", "400")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
    ...
    write_msg(
        ...
        "textDocument":{"uri":uri,"languageId":"kamailio-cfg","version":7,"text":"request_route { exit; }\n"}}}),
    );
    let d = wait_for(
        &rx,
...
fn midflight_buffer_change_suppresses_the_stale_result() {
    ...
    let mut child = Server::new(
        Command::new(env!("CARGO_BIN_EXE_kamailio-lsp"))
            .env("KAMAILIO_LSP_BIN", stub.display().to_string())
            .env("KAMAILIO_LSP_CHECK_TIMEOUT_MS", "5000")
            // make every check slow enough to race
            .env("KAMAILIO_LSP_TEST_CHECK_DELAY_MS", "700")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
    ...
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","method":"textDocument/didOpen","params":{
        "textDocument":{"uri":uri,"languageId":"kamailio-cfg","version":1,"text":"request_route { exit; }\n"}}}),
    );
...
fn output_flood_is_capped_and_clears_diagnostics() {
    let dir = std::env::temp_dir().join(format!("kamlsp-flood-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    ...
    std::fs::write(
        &stub,
        "#!/bin/sh\ncfg=\"\"\nfor a in \"$@\"; do cfg=\"$a\"; done\nhead -c 8388608 /dev/zero | tr '\\0' 'A' >&2\necho \" 0(1) CRITICAL: <core> [core/cfg.y:1]: yyerror_at(): parse error in config file $cfg, line 1, column 1-2: after flood\" >&2\nexit 255\n",
    )
    .unwrap();
    use std::os::unix::fs::PermissionsExt;
    let mut p = std::fs::metadata(&stub).unwrap().permissions();
    ...
    let mut child = Server::new(
        Command::new(env!("CARGO_BIN_EXE_kamailio-lsp"))
            .env("KAMAILIO_LSP_BIN", stub.display().to_string())
            .env("KAMAILIO_LSP_OUTPUT_CAP_BYTES", "65536")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
