...
/// a JSON body, and `.get("kamailioLsp")` unwraps the namespace. A
...
fn server_options() -> BTreeSet<String> {
    const NOT_SETTINGS: &[&str] = &["route", "kamailioLsp"];
...
fn the_option_scan_sees_every_read_site() {
    let src = read("src/server.rs");
    const NOT_SETTINGS: &[&str] = &["route", "kamailioLsp"];
    let scanned = server_options();

    // This server reads its settings in TWO places — the
    // initialization options and the live configuration change — so a
    // setting has more read SITES than it has names, and counting
    // sites against names compares two different things. What must
    // hold is that no site is invisible to the scan.
    let mut unseen: Vec<String> = Vec::new();
    let mut sites = 0usize;
    for (i, _) in src.match_indices(".get(\"") {
        let Some((name, _)) = src[i + 6..].split_once('"') else {
            continue;
        };
        sites += 1;
        if !NOT_SETTINGS.contains(&name) && !scanned.contains(name) {
            unseen.push(name.to_string());
        }
    }
    // POSITIVE CONTROL: the file was read and does have read sites,
    // so an empty `unseen` is a measurement and not an empty scan.
    assert!(sites >= 10, "only {sites} read site(s) found");
    assert!(
        unseen.is_empty(),
        "these are read and the scan does not see them, so every gate built on \
         it passes for the wrong reason: {unseen:?}"
    );
}
...
fn every_documented_setting_still_exists() {
    ...
    for line in features
        .lines()
        .filter(|l| l.trim_start().starts_with("| `kamailioLsp."))
    {
