...
fn the_manifest_declares_the_command_and_a_key_sequence() {
    ...
    let toggle = cmds
        .iter()
        .find(|c| c["command"] == "kamailioLsp.toggleAssistance")
    ...
    let bind = keys
        .iter()
        .find(|k| k["command"] == "kamailioLsp.toggleAssistance")
...
fn the_setting_exists_and_defaults_to_on() {
    let m = manifest();
    let p = &m["contributes"]["configuration"]["properties"]["kamailioLsp.assistance"];
...
fn the_toggle_is_not_in_the_restart_list() {
    ...
    assert!(
        ext[start..end].contains("kamailioLsp.serverPath"),
...
fn boot(
    ...
    let base = std::env::temp_dir().join(format!("kalsp-assist-{tag}-{}", std::process::id()));
    ...
    let mut opts = serde_json::json!({ "kamailioPath": "" });
    ...
    let mut child = Server::new(
        Command::new(env!("CARGO_BIN_EXE_kamailio-lsp"))
            ...
            .env_remove("KAMAILIO_LSP_ASSISTANCE")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
...
fn the_namespaced_payload_shape_toggles_too() {
    ...
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","method":"workspace/didChangeConfiguration","params":{
            "settings":{"kamailioLsp":{"assistance": false}}}}),
    );
    let mut id = 3;
    let off = loop {
        if hover(&mut stdin, &rx, &uri, id).is_null() {
