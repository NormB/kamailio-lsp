...
//! checker something `kamailio -c` was never meant to accept.  The
...
fn start(
    ...
    start_in(base, kamailio_path, true)
}
...
fn start_in(
    ...
    write_msg(
        ...
        &serde_json::json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{
            "capabilities":{},
            "initializationOptions":{"kamailioPath": kamailio_path},
...
fn the_server_names_the_root_a_fragment_belongs_to() {
    ...
    // which is how the client tells "kamailio config" from "some .cfg"
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","id":3,"method":"kamailio/analysisRoot",
            "params":{"uri": root_uri}}),
    );
    ...
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","id":4,"method":"kamailio/analysisRoot",
...
fn a_fragment_opened_alone_is_analysed_in_its_roots_closure() {
    ...
    let items = v["params"]["diagnostics"].as_array().unwrap();
...
fn navigation_from_a_fragment_reaches_the_root_that_includes_it() {
    ...
    assert_eq!(v["result"]["range"]["start"]["line"], 2);
...
fn the_checker_runs_on_the_root_and_the_fragments_errors_come_back_to_it() {
    ...
    // the FRAGMENT, spelled the way kamailio spells an include — the
    ...
    std::fs::write(
        &stub,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > {}\necho \" 0(1) CRITICAL: <core> [core/cfg.y:4048]: yyerror_at(): parse error in config file inc/routes.cfg, line 3, column 5-9: planted error\" >&2\nexit 255\n",
            argv_file.display()
        ),
    )
    .unwrap();
        ...
        |v| {
            v["method"] == "textDocument/publishDiagnostics"
                ...
                && v["params"]["diagnostics"]
                    .as_array()
                    .is_some_and(|a| a.iter().any(|d| d["source"] == "kamailio -c"))
        },
    ...
    assert!(
        args.iter().any(|a| a.ends_with("kamailio.cfg")),
...
fn adding_an_include_clears_the_fragment_it_now_belongs_to() {
    ...
    std::fs::write(base.join("kamailio.cfg"), &root_without).unwrap();
    let (mut child, rx, mut stdin) = start(&base, "");
    did_open(&mut stdin, &frag_uri, FRAG_TEXT);
    let v = wait_for(
        &rx,
...
fn a_fragment_outside_the_roots_closure_keeps_its_own_includes() {
    ...
    const N: usize = 90;
    let mut root = String::from("#!KAMAILIO\n");
    for i in 0..N {
        root.push_str(&format!("include_file \"c{i}.cfg\"\n"));
    }
    root.push_str("route[TOP] {\n    exit;\n}\n");
    std::fs::write(base.join("kamailio.cfg"), &root).unwrap();
    for i in 0..N - 1 {
        std::fs::write(
...
fn every_request_answers_the_same_shape_split_as_whole() {
    ...
    std::fs::write(whole_dir.join("kamailio.cfg"), &whole).unwrap();
    std::fs::write(split_dir.join("kamailio.cfg"), &root).unwrap();
    std::fs::write(split_dir.join("inc/routes.cfg"), &frag).unwrap();
    let whole_uri = format!("file://{}", whole_dir.join("kamailio.cfg").display());
    ...
    write_msg(
        ...
        &serde_json::json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{
            "capabilities":{},
            "initializationOptions":{"kamailioPath":""},
            "workspaceFolders":[
...
/// Hostile and malformed URIs to `kamailio/analysisRoot`.
///
...
fn analysis_root_survives_whatever_the_editor_hands_it() {
    ...
    for uri in &hostile {
        ...
        write_msg(
            &mut stdin,
            &serde_json::json!({"jsonrpc":"2.0","id":id,"method":"kamailio/analysisRoot",
                "params":{"uri": uri}}),
        );
        let want = id;
...
fn a_config_the_scan_accepts_is_a_config_the_closure_can_load() {
    ...
    let root_path = base.join("kamailio.cfg");
...
fn a_root_too_large_to_load_is_not_claimed_and_is_announced() {
    ...
    assert!(
        v["params"]["message"]
            .as_str()
            .is_some_and(|m| m.contains("kamailio-lsp")),
...
fn encodings_do_not_change_what_a_fragment_is_part_of() {
    for (tag, eol, bom) in [
        ...
        std::fs::create_dir_all(base.join("inc")).unwrap();
        let root = format!(
            "{bom}#!KAMAILIO\ninclude_file \"inc/f.cfg\"{eol}route[TOP] {{{eol}    exit;{eol}}}{eol}"
        );
        let frag =
...
fn the_filesystem_being_hostile_does_not_take_the_server_down() {
    ...
    for name in [
        ...
        write_msg(
            &mut stdin,
            &serde_json::json!({"jsonrpc":"2.0","id":50,"method":"kamailio/analysisRoot",
                "params":{"uri": uri}}),
        );
    ...
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","id":51,"method":"kamailio/analysisRoot",
            "params":{"uri": f_uri}}),
    );
    ...
    std::fs::remove_file(base.join("kamailio.cfg")).unwrap();
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","method":"workspace/didChangeWatchedFiles","params":{
            "changes":[{"uri": format!("file://{}", base.join("kamailio.cfg").display()), "type":3}]}}),
    );
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","id":52,"method":"kamailio/analysisRoot",
            "params":{"uri": f_uri}}),
    );
    let v = wait_for(
        &rx,
...
fn the_server_survives_a_client_behaving_badly() {
    ...
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","id":id,"method":"kamailio/notARealMethod",
            "params":{}}),
    );
    let want = id;
...
fn repeated_questions_do_not_drift() {
    ...
    assert!(answers.iter().next().unwrap().contains("kamailio.cfg"));
...
fn every_setting_behaves_the_same_for_a_fragment() {
    ...
    let boot = |base: &std::path::Path, opts: serde_json::Value| {
        ...
        let rx = spawn_reader(&mut child);
        let mut stdin = child.stdin.take().unwrap();
        let mut init = serde_json::json!({"kamailioPath": ""});
    ...
    write_msg(
        ...
        &serde_json::json!({"jsonrpc":"2.0","method":"workspace/didChangeConfiguration",
            "params":{"settings":{"kamailioLsp":{"analyzerDiagnostics": false}}}}),
    );
    let v = wait_for(
        &rx,
        |v| {
    ...
    write_msg(
        &mut stdin,
        &serde_json::json!({"jsonrpc":"2.0","method":"workspace/didChangeConfiguration",
            "params":{"settings":{"kamailioLsp":{"analyzerDiagnostics": true}}}}),
    );
    let v = wait_for(
        &rx,
        |v| {
