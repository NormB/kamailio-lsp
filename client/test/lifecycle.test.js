...
const NAMESPACE = 'kamailioLsp';
const BINARY_SETTING = 'kamailioPath';
...
(async () => {
    ...
    // `kamailio -c` dlopens the modules a config loads, so checking a
    ...
    reset();
    trusted = true;
    settings[BINARY_SETTING] = '/usr/sbin/kamailio';
    ext.activate(ctx());
    await settle();
    assert.strictEqual(
        lastInit()[BINARY_SETTING],
    ...
    // wrong. Until then nothing told the reader which Kamailio
    // release their file was being parsed against.
    reset();
    ext.activate(ctx());
    await settle();
    const announce = clients[clients.length - 1].handlers['kamailioLsp/catalogue'];
    assert.ok(announce, 'the client must subscribe to the catalogue announcement');

    // nothing to show before the server has said anything
    activeEditor = { document: { languageId: 'kamailio-cfg' } };
    ...
    announce({ describe: 'Kamailio 6.1.4 (built in)', version: '6.1.4' });
    assert.strictEqual(statusItem.shown, true, 'it appears once announced');
    assert.ok(
        statusItem.text.includes('Kamailio 6.1.4'),
        `the release must be visible: ${statusItem.text}`,
    );

    // and it is not shown over someone else's file
    ...
    // a configured tree names the tree, not a version
    activeEditor = { document: { languageId: 'kamailio-cfg' } };
    handlers.activeEditor?.();
    ...
    await handlers.config({
        affectsConfiguration: (s) => s === NAMESPACE || s.endsWith('.kamailioVersion'),
    });
    await settle();
    assert.ok(
    ...
    ext.activate(ctx());
    await settle();
    activeEditor = { document: doc('/w/kamailio.cfg', 'kamailio-cfg') };
    handlers.activeEditor?.();
    assert.ok(
