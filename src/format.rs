//! Line-preserving whitespace formatter for `kamailio.cfg`.
//!
...
//! * **`#!` and `!!` preprocessor directives.** Kamailio's
...
impl Default for Options {
    /// Tabs, matching the upstream `etc/kamailio.cfg` convention, for
...
fn formatted_lines(text: &str, opts: &Options) -> Vec<String> {
    ...
    for line in &lines {
        ...
        let continues_literal = line
            ...
            .is_some_and(|c| matches!(c, Class::Str | Class::Comment | Class::Preproc));
