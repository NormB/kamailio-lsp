...
pub fn run_check(args: &[String]) -> i32 {
    ...
    // green here and warned about in the editor. `KAMAILIO_LSP_SRC` is
    // the same fallback the server reads, so both resolve one source.
    // The wiki is not consulted: it carries core docs, and this check
    // is about what a MODULE exports.
    let (modules, origin) = match std::env::var("KAMAILIO_LSP_SRC")
        .ok()
        .filter(|s| !s.trim().is_empty())
    {
        Some(src) => {
            ...
            if harvested.is_empty() {
                eprintln!(
                    "kamailio-lsp: '{src}' yields no module documentation — \
        ...
        None => {
            let wanted = std::env::var("KAMAILIO_LSP_VERSION")
                .ok()
    ...
    eprintln!(
        "kamailio-lsp: modparam checks against {}",
        origin.describe()
    );
